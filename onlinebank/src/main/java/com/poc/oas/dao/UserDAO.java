...
public class UserDAO implements IUserDAO {
		...
		try {
			...
			jdbcTemplate.update(new PreparedStatementCreator() {
				...
				public PreparedStatement createPreparedStatement(Connection conn)
						throws SQLException {
					// TODO Auto-generated method stub
					PreparedStatement ps = conn.prepareStatement(createAddress,
					...
					ps.setLong(10, userParam.getAddress()
							.getDataImportHistoryId());
					ps.setString(11, userParam.getAddress()
							.getSecondaryPhoneExt());
					ps.setString(12, userParam.getAddress()
							.getPrimaryPhoneExt());
					ps.setInt(13, 1);
					return ps;
				}
			...
			logger.info("Generated address id:"
					+ addressKey.getKey().longValue());
	...
	public List<String> fetchUserName(String userName) {
		...
		String queryForUsername = "SELECT USER_NAME FROM USERS WHERE USER_NAME like ?";
		try {
			userNameList = jdbcTemplate.query(queryForUsername,
					new Object[] { "%"+userName+"%" }, new UserNameMapper());
			logger.info("List of user names " + userNameList);
